...
**2026-08-22 기준으로 끝난 것**

- **단별 영상 링크** — 표의 시각을 누르면 그 단이 시작되는 지점부터 영상이 재생됩니다.
  저장된 도안 7건 전부 100%이며, Knotty를 만든 이유 그 자체인 기능입니다
- **코수 검증** — 단별 약어와 코수가 맞는지 서버가 계산해 대조하고, 이상한 단만 ⚠️로 표시합니다.
  값을 자동으로 고치지는 않습니다. 확실할 때만 경고하도록 부분 편물·묶음 단은 검사에서 뺐습니다
- **기법 사전 연동** — `craft_terms` 36건을 프롬프트에 주입해 약어를 통일하고, 도안 기호 SVG를 함께 보여줍니다
- **호출량 방어** — 허용 출처 제한 + IP당 한도 + 전역 일일 상한. AI가 돌지 않은 요청은 세지 않습니다
- **원작자에게 되돌려 보내기** — 설명란에서 도안·준비물 구매 링크를 뽑아 카드에 배치합니다.
  "도안을 만든다" 대신 **"영상을 정리한다"**로 카피를 바꿨고, 카드에 원본 영상과 함께 보라는 고지를 넣었습니다
...
**AI가 돌지 않은 요청은 세지 않습니다.** 캐시 적중, 자막 수집 실패, Gemini가 응답하지 못한 경우(429·503)는
카운트를 되돌려 줍니다. 한도는 비용을 막으려고 두는 것이므로, 자막 없는 영상을 몇 번 시도했다는 이유로
한 시간 잠기는 것은 벌을 잘못 주는 것입니다.
